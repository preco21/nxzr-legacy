use super::helper;
use crate::{device::DeviceConfig, shared::SystemCommandError};
...
pub enum DeviceInternalError {
    ...
    SystemCommand(SystemCommandError),
}
...
impl From<SystemCommandError> for DeviceError {
    fn from(err: SystemCommandError) -> Self {
...
impl Device {
    ...
    pub async fn ensure_adapter_address_switch(self) -> Result<Self, DeviceError> {
        ...
        if &addr[..3] != SWITCH_MAC_PREFIX {
            ...
            helper::set_adapter_address(adapter_name.as_str(), new_addr.into()).await?;
