...
impl InputReport {
    ...
    pub fn with_bytes(data: impl AsRef<[u8]>) -> ReportResult<Self> {
        let cloned_data = data.as_ref().to_vec();
        if cloned_data.len() < 1 {
            return Err(ReportError::TooShort);
        }
        if cloned_data[0] != 0xA1 {
            return Err(ReportError::Malformed);
        }
        Ok(Self { data: cloned_data })
    }
...
impl OutputReport {
    ...
    pub fn subcommand_data(&self) -> Option<&[u8]> {
        if self.data.len() < 13 {
            return None;
        }
        Some(&self.data[12..])
